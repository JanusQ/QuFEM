...
class Simulator():
    def __init__(self, n_qubits,prob_1,prob_2):

        noise_model = NoiseModel()
        # noise_model.add_all_qubit_readout_error()
        # prob_1 = 0.03    # 1-qubit gate
        # prob_2 = 0.048   # 2-qubit gate
        # prob_1 = 0.02
        # prob_2 = 0.03
    ...
    def execute(self, circuit: List[QuantumCircuit], n_samples: int = 10000) -> List[dict]:
        ...
        result = self.simulator.run(circuit, shots=n_samples, basis_gates = ['rx', 'ry', 'rz', 'cx']).result().get_counts()
