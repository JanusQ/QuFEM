...
@ray.remote
def construct_columns(items, n_qubits):
    columns = {}
    for qubit in range(n_qubits):
        columns[f'{qubit}_set'] = []
        columns[f'{qubit}_read'] = []
    columns['count'] = []
    for real_bitstring, status_count in items:
        for measure_bitstring, count in status_count.items():
            for qubit in range(n_qubits):
                columns[f'{qubit}_set'].append(int(real_bitstring[qubit]))  # 改成了int，数据量小一些
                columns[f'{qubit}_read'].append(int(measure_bitstring[qubit]))
            columns['count'].append(count)
    return columns

class PdBasedProtocolResults():
    def __init__(self, protocol_results: dict, n_qubits: int, multi_process: bool = False):
        ...
        if multi_process:
            ...
            futures = []
            for items in batch([item for item in protocol_results.items()], batch_size = len(protocol_results)//120): #120是hard code的cpu数
                futures.append(construct_columns.remote(items, n_qubits))
                # break
                
            for sub_columns in ray.get(futures): #wait(futures, show_progress = True):
                for qubit in range(n_qubits):
                    columns[f'{qubit}_set'] += sub_columns[f'{qubit}_set']
                    columns[f'{qubit}_read'] += sub_columns[f'{qubit}_read']
                columns['count'] += sub_columns[f'count']

        else:
            for real_bitstring, status_count in tqdm(protocol_results.items()):                                      #16min
                for measure_bitstring, count in status_count.items():
                    for qubit in range(n_qubits):
                        columns[f'{qubit}_set'].append(int(real_bitstring[qubit])) # set的值 0, 1, 2 (没有set)
                        columns[f'{qubit}_read'].append(int(measure_bitstring[qubit])) # 读到的值 0, 1, 2 (不读)
                        # columns[f'{qubit}_measure'].append(real_bitstring[qubit] != '2') # 是否进行读取 0, 1
                    columns['count'].append(count)

        print('finish columns construction', len(columns['count']))
        
        # 全是2的没有实际测量所以这里直接加了一个
        for qubit in range(n_qubits):
            columns[f'{qubit}_set'].append('2') # set的值 0, 1, 2 (没有set)
            columns[f'{qubit}_read'].append('2') # 读到的值 0, 1, 2 (不读)
        columns['count'].append(5000) # 可能还得设一个和采样数一样的数值

        
        self.df = pd.DataFrame(data=columns)
        
        # index = [f'{qubit}_set' for qubit in range(n_qubits)] + [f'{qubit}_read' for qubit in range(n_qubits)]
        # self.df.set_index(index, inplace=True, append = True)
        print('finish DataFrame construction')
    ...
    def __getitem__(self, qubit_set_values):
        ...
        for qubit, value in qubit_set_values:
            filter_df = filter_df[filter_df[f'{qubit}_set'] == value]
        ...
        #         condition = self.df[f'{qubit}_set'] == value
        #     else:
        #         condition = condition & self.df[f'{qubit}_set'] == value
        # filter_df = self.df[condition]
        # print(time.time() - start_time)
...
def construct_cpd(qubit, related_qubits, protocol_results):
    ...
    for set_types in itertools.product(*[(0, 1, 2)]*len(related_qubits)):
        ...
        if len(filter_df) == 0:
            print('Wanrning!!! filter_df is empty for', set_types, related_qubits, qubit)
        
        # 这里面要不要改成int
        for read_type in (0, 1, 2):  # 对应1,2,3的操作
            data[read_type][colum_index] = filter_df[filter_df[f'{qubit}_read'] == read_type]['count'].sum()
            # print(elm)
...
def construct_bayesian_network(protocol_results: PdBasedProtocolResults, n_qubits, groups):
    ...
    for qubit in range(n_qubits):
        ...
        # futures.append(construct_cpd.remote(qubit, related_qubits, protocol_results))
        futures.append(construct_cpd(qubit, related_qubits, protocol_results))

        
        # futures.append(executor.submit(construct_cpd, qubit, related_qubits, protocol_results))

    for qubit_cpd in wait(futures, show_progress=True): #tqdm(as_completed(futures)):
