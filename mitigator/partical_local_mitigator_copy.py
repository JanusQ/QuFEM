...
class ParticalLocalMitigator():
    ...
    def characterize_M(self, groups: List[List[int]]):
        ...
        for group in groups:
            ...
            M = np.array([[102,2,1,5],[3,101,8,1],[3,9,101,1],[3,9,6,104]],dtype=np.float32)
...
# def recursive_merge(groups, group2invM, group2basis, threshold):
#     if len(groups) < 100:
#         now_basis = None  #basis_1q[basis[0]]
#         now_values = None
...
#             group_basis = group2basis[group] #basis[pointer: pointer + group_size]
#             group_mitigated_vec = invM[:,list2int(group_basis)]
#             group_basis = np.arange(2**group_size)
            
#             if now_basis is None:
#                 now_basis = group_basis
#                 now_values = group_mitigated_vec
#             else:
#                 next_basis = kron_basis(now_basis, group_basis, group_size)
#                 next_values = np.kron(now_values, group_mitigated_vec)
#                 filter = np.logical_or(next_values > threshold, next_values < -threshold)
#                 now_basis = next_basis[filter]
#                 now_values = next_values[filter]
#         return now_basis, now_values
#     else:
#         g1, g2 = groups[:len(groups)/2], groups[len(groups)/2:]
#         now_basis1, now_values1 = recursive_merge(g1, group2invM, group2basis, threshold)
#         now_basis2, now_values2 = recursive_merge(g2, group2invM, group2basis, threshold)
        
#         sqrt_threshold = np.sqrt(threshold)
        
#         def filter(now_basis, now_values):
#             abs_now = np.abs(now_values) 
#             now_values_large = now_values[abs_now >= sqrt_threshold]
#             now_values_small = now_values[abs_now < sqrt_threshold]
#             return now_values_large, now_values_small
        
#         next_basis = kron_basis(now_basis, group_basis, group_size)
#         next_values_large = np.kron(now_values_large, group_mitigated_vec)
#         now_values_small = now_values_small * group_mitigated_vec[np.abs(group_mitigated_vec) > 0.03][0]
#         now_values = np.concatenate((next_values_large, now_values_small))
#         now_basis = now_basis[:now_basis.size]
