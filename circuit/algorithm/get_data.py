...
def get_data(id, qiskit_circuit, coupling_map, mirror, trans = True):
    ...
    # if trans == True:
    #     qiskit_circuit = transpile(new_qiskit_circuit, coupling_map = coupling_map, optimization_level=3,inst_map = list(i for i in range(18)))
    # else:
...
def get_dataset_bug_detection(min_qubit_num, max_qubit_num,coupling_map, mirror):
    ...
    for n_qubits in range(min_qubit_num, max_qubit_num):
        ...
        dataset.append(get_data(f'bernstein_vazirani', al.bernstein_vazirani(get_bitstr(n_qubits - 1)), coupling_map, mirror))  #1
        
        
        al = Algorithm(n_qubits)
        # al2 = Algorithm(8)
        dataset.append(get_data(f'hamiltonian_simulation', hamiltonian_simulation.get_cir(n_qubits), coupling_map, mirror)) #2
        # dataset.append(get_data(f'ising', ising.get_cir(n_qubits), coupling_map, mirror)) #2
        # algorithm.append(get_data(f'QAOA_maxcut', QAOA_maxcut.get_cir(n_qubits)))
        # dataset.append(get_data(f'qknn', qknn.get_cir(n_qubits), coupling_map, mirror)) #2
        dataset.append(get_data(f'qsvm', qsvm.get_cir(n_qubits), coupling_map, mirror)) #2
        dataset.append(get_data(f'vqc', vqc.get_cir(n_qubits), coupling_map, mirror)) #1
        # dataset.append(get_data(f'qft', al.qft(), coupling_map, mirror)) #1
        dataset.append(get_data(f'ghz', al.ghz(), coupling_map, mirror)) #1
        ...
        dataset.append(get_data(f'deutsch_jozsa', deutsch_jozsa.get_cir(n_qubits - 1, get_bitstr(n_qubits - 1)), coupling_map, mirror)) #1
        # if n_qubits % 5 == 0:
        ...
        if n_qubits % 2 == 0:
            dataset.append(get_data(f'simon', simon.get_cir(get_bitstr(n_qubits // 2)), coupling_map, mirror))   #1
