...
# n = 10
# p = 0.2
# x = jax.random.bernoulli(key, p, shape=(1000,))
...
def rr(qc_sample, measure_fids, n_rm_sample = 100):
    ...
    key = jax.random.PRNGKey(random.randint())
    
    def _rr_sample_construct_prob(measure_fid, sample):
        transfer_p = 1 - measure_fid[inverse(sample)] # 是另外一个态转移来的概率
        transfer = jax.random.bernoulli(jax.random.PRNGKey(random.randint()), transfer_p, shape=(n_rm_sample,))
    ...
    def rr_sample(sample, measure_fids):
        # transfers = np.array([
        #     _rr_sample_construct_prob(elm1, elm2)
        #     for elm1, elm2 in zip(measure_fids, sample)
        # ])
        transfers = vmap(_rr_sample_construct_prob, in_axes=(0, 0))(measure_fids, sample)
        
        rm_samples = [
            _rr_sample_mit(sample, transfers[:,index])
            for index in range(transfers.shape[1])
        ]
        # rm_samples = vmap(_rr_sample_mit, in_axes=(None, 1))(sample, transfers)
        return rm_samples
